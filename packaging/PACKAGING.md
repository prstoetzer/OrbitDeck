...
```bash
sudo apt install ../orbitdeck_0.39.2-1_all.deb   # pulls deps + the full optional set
orbitdeck
```
...
git archive --format=tar.gz --prefix=OrbitDeck-0.39.2/ \
    -o ~/rpmbuild/SOURCES/orbitdeck-0.39.2.tar.gz v0.39.2
...
```bash
sudo dnf install ~/rpmbuild/RPMS/noarch/orbitdeck-0.39.2-1.*.noarch.rpm
orbitdeck
```
...
`makepkg` produces `orbitdeck-0.39.2-1-any.pkg.tar.zst`. The recipe pulls
...
git commit -m "Initial import: orbitdeck 0.39.2-1"
git push
```
...
# 3. commit + push
git commit -am "Update to 0.39.2-1"
git push
```
...
- The `v0.39.2` **tag must be live and public on GitHub** or `updpkgsums` and
...
| --- | --- | --- |
| Build standalone apps | `.github/workflows/build.yml` | PyInstaller bundles: Windows, macOS (Apple Silicon), Linux x86_64, Raspberry Pi OS arm64 — **plus a separate single-file OrbitTerm** for Windows, macOS arm64 and x86_64, Linux x86_64 and Pi arm64 |
...
## Which packages include OrbitTerm

| Package | OrbitTerm | How |
| --- | --- | --- |
| `.deb` | yes | pybuild installs from `pyproject.toml`, so the `orbitterm` console script comes with it |
| `.rpm` | yes | the spec lists `%{python3_sitelib}/orbitterm/` and `%{_bindir}/orbitterm` explicitly |
| Arch | yes | `python -m installer` on the wheel, which carries both entry points |
| AppImage | yes | `AppRun` has a dedicated path: `./OrbitDeck.AppImage term` |
| Flatpak | yes, but | installed in the sandbox; only `orbitdeck` is the exported command, so use `flatpak run --command=orbitterm io.github.prstoetzer.OrbitDeck` |
| PyInstaller desktop bundle | yes | ships an `OrbitTerm` binary beside `OrbitDeck` in the same folder (~7 MB on a ~350 MB bundle, since the dependencies are already there) |
| PyInstaller OrbitTerm build | yes | that *is* OrbitTerm: one file, console mode, ~9 MB |

The pip wheel and sdist carry both entry points (`orbitdeck` and `orbitterm`).
