...
```bash
sudo apt install ../orbitdeck_0.35.4-1_all.deb   # pulls deps + the full optional set
orbitdeck
```
...
git archive --format=tar.gz --prefix=OrbitDeck-0.35.4/ \
    -o ~/rpmbuild/SOURCES/orbitdeck-0.35.4.tar.gz v0.35.4
...
```bash
sudo dnf install ~/rpmbuild/RPMS/noarch/orbitdeck-0.35.4-1.*.noarch.rpm
orbitdeck
```
...
`makepkg` produces `orbitdeck-0.35.4-1-any.pkg.tar.zst`. The recipe pulls
...
git commit -m "Initial import: orbitdeck 0.35.4-1"
git push
```
...
- The `v0.35.4` **tag must be live and public on GitHub** or `updpkgsums` and
