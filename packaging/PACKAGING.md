...
```bash
sudo apt install ../orbitdeck_0.39.4-1_all.deb   # pulls deps + the full optional set
orbitdeck
```
...
git archive --format=tar.gz --prefix=OrbitDeck-0.39.4/ \
    -o ~/rpmbuild/SOURCES/orbitdeck-0.39.4.tar.gz v0.39.4
...
```bash
sudo dnf install ~/rpmbuild/RPMS/noarch/orbitdeck-0.39.4-1.*.noarch.rpm
orbitdeck
```
...
`makepkg` produces `orbitdeck-0.39.4-1-any.pkg.tar.zst`. The recipe pulls
...
git commit -m "Initial import: orbitdeck 0.39.4-1"
git push
```
...
# 3. commit + push
git commit -am "Update to 0.39.4-1"
git push
```
...
- The `v0.39.4` **tag must be live and public on GitHub** or `updpkgsums` and
