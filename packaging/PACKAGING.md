...
```bash
sudo apt install ../orbitdeck_0.36.9-1_all.deb   # pulls deps + the full optional set
orbitdeck
```
...
git archive --format=tar.gz --prefix=OrbitDeck-0.36.9/ \
    -o ~/rpmbuild/SOURCES/orbitdeck-0.36.9.tar.gz v0.36.9
...
```bash
sudo dnf install ~/rpmbuild/RPMS/noarch/orbitdeck-0.36.9-1.*.noarch.rpm
orbitdeck
```
...
`makepkg` produces `orbitdeck-0.36.9-1-any.pkg.tar.zst`. The recipe pulls
...
git commit -m "Initial import: orbitdeck 0.36.9-1"
git push
```
...
# 3. commit + push
git commit -am "Update to 0.36.9-1"
git push
```
...
- The `v0.36.9` **tag must be live and public on GitHub** or `updpkgsums` and
