...
|---|---|---|
| `pip install "orbitdeck[accurate]"` | `sgp4` — the C-accelerated full SGP4/SDP4 backend | The bundled pure-Python propagator is used instead. It is accurate to well under a kilometer for typical LEO passes, but it is **slower** for the heavy repeated propagation behind pass- and node-finding, and its **deep-space (GEO/HEO/Molniya) terms are only approximate**. |
...
| 6 | **Sky at a Glance** | A Gantt timeline of every upcoming pass across all favorites, so you can see at a glance where the busy stretches and the long quiet gaps fall. |
| 7 | **Pass Detail** | Polar sky-track plus an elevation-vs-time profile for a chosen pass. |
| 8 | **Ground Track** | Forward ground track over the next 1, 3, 5, or 8 upcoming orbits. |
| 9 | **Pass Progression** | One satellite's passes across 10+ days as a scrollable stack of 24-hour timelines — each pass placed at its time of day, width = duration, shaded by max elevation. |
| 10 | **Orbital Analysis** | Eleven pages (see the manual): Info, Live, Next Pass, Ground Track, Doppler, Nodal, Sun/Beta, Pass Outlook, Position, EQX Map, and EQX List. Clean grouped data cards and plots. The EQX List exports the equator-crossing schedule to CSV. |
| 11 | **Orbital History** | The Space-Track element archive in four views — **value**, **rate** of change, a **rate analysis** that reports whether the rate has itself changed (with maneuver-jump detection), and a **summary table** — with time-axis zoom and pan. |
| 12 | **Illumination** | A **raster** tab: scrollable 30-day sunlit-vs-eclipse map (prints a 60-day summary with mean eclipse fraction). An **Eclipse table** tab: orbit-by-orbit umbral eclipse list (enter/exit/duration/interval/sun-angle) and a daily summary (total, longest, percent of day, beta angle) over a selectable 1–14 day span, exportable to CSV and a printable PDF report. |
| 13 | **Orbital Zones** | Transit windows and dwell for the South Atlantic Anomaly, the radiation belts, the polar caps and eclipse. Belt verdicts use a tilted dipole rather than IGRF, so they are indicative near the horns and inside the SAA. |
| 14 | **AO-7 Mode** | Mode A/B phase estimate for AO-7 from AMSAT community reports, with the illumination state that gates it, fit confidence, report agreement and how many mode changes were seen. |
...
| 16 | **Sun/Moon Transits** | Solar and lunar disk crossings and near approaches for the selected satellite — the shots worth setting up for. |
| 17 | **Astronomy** | Observing astronomy on the same Sun, Moon and planet models the rest of the app uses: the eleven major **meteor showers** with radiant elevation at scatter prime time, **Jupiter** decametric Io-A/B/C windows, **aurora** likelihood from magnetic latitude and Kp, **twilight** crossings, **EME conditions** across the lunar month, and lunar **occultations** and planetary **appulses** a year ahead. |
| 18 | **Sky Map** | The selected satellite's track against the star background, for visual acquisition and photography planning. |
| 19 | **Conjunctions** | Close approaches between the selected satellite and another catalog object, with miss distance and relative velocity. |
| 20 | **Workable** | What's inside the footprint — **grids**, **US states**, or **DXCC entities** — live (now) or unioned across the next pass, for grid/state/DX chasing. Exports the current list to CSV. |
| 21 | **Radio** | Pick an **upcoming pass** to plan against, then get a **link budget** (free-space path loss, propagation delay, estimated received power, with separate **your-station** and **satellite** (TX power + antenna gain) parameters) with a **time-in-pass scrubber** that evaluates the geometry anywhere from AOS to LOS (snap to TCA), plus range-rate and downlink-Doppler readouts. A **Doppler tuning playbook** gives a per-pass table of corrected RX/TX frequencies (with antenna **az/el** at each step) at a chosen interval, with its own **passband-position** slider so a linear bird's table is built around where you're tuned. For a linear transponder worked full duplex you can **hold the uplink OR the downlink fixed** and the other leg is round-trip corrected so you keep hearing yourself. Export the playbook to CSV or a printable PDF sheet. |
| 22 | **Planning** | Goal-directed planning: **best time to work a target** (grid square, US state, DXCC entity, or lat/lon) by finding windows when you and the target share the footprint; **visible passes** with estimated optical magnitude and a twilight-darkness filter; **satellite-to-satellite** line-of-sight windows; a **rove route planner** (enter your planned grid stops with optional time-window hints and get, per stop, the covering passes and the US states / DXCC entities / grids workable through each, for the selected satellite **or all your favorites at once** — exportable to CSV and a printable rove sheet); and an **element-set trust** panel (epoch age, trust level, along-track drift estimate). Results export to CSV. |
| 23 | **Tools** | 49 calculators across six categories: general (scientific, programmer, unit, character/byte lookup, DXCC), antennas and feedline, link budget and RF exposure, orbital mechanics, and terrain. |
| 24 | **Graphing Calc** | A two-trace expression plotter for quick what-if curves. |
| 25 | **Tiny BASIC** | The CardSat Tiny BASIC dialect, source-compatible in both directions: a program written on the card runs here unchanged. Graphics keep the card's 240×135 space and ten colors so pictures match exactly, and the desktop display scales by a whole number of pixels. |
| 26 | **Activations / QRZ** | Upcoming satellite activations from hams.at with a **can I work it?** check that finds the mutual window, shows both stations' sky tracks, and opens a **DX Doppler** table seeded from the activation's own satellite, transponder and stated frequency. Plus QRZ callsign lookup. |
| 27 | **AMSAT Status** | The community status board, and one-key reporting of what you heard (public and attributed). |
| 28 | **OSCARLOCATOR Sim** | An interactive on-screen OSCARLOCATOR: **drag the map** to rotate the path-arc overlay over a polar or QTH base map and watch the satellite position and QTH footprint move, without printing transparencies. The map has a **protractor-style rim** with per-degree tick marks and longitude / azimuth labels. Drive it live, **by hand (drag the disc to any equator-crossing longitude, and drag near the moving dot to step the minutes after the crossing)**, or seed it to the next pass; a compact next-equator-crossings list is built in. A **lab-satellite mode** lets you invent a hypothetical satellite and edit its orbital elements in a pop-up — with live explainers, preset orbits, an A/B comparison ghost, a guided tour, and a glossary — then hand-position, name, and print it as an OSCARLOCATOR exactly like a catalog bird. Exports the matching printable PDF. |
| 29 | **Learn** | A home for OrbitDeck's teaching tools, organized into five groups (**Orbits**, **Geometry**, **Passes**, **Radio**, **Reference**) selected from a category row, with a **"use a lab orbit" toggle** so the orbit tools can run against a satellite you design. **Orbits:** a **Kepler** equal-areas demo, an **Anomalies** mean-vs-true visualizer, a **Speed** (vis-viva) plot, a **Transfers** Hohmann delta-v calculator, an **element-age** view, and a **Decay** lifetime-vs-altitude curve. **Geometry:** **Slant range** vs elevation, a **Horizon** reach view, a **Track drift** westward-shift view, a **Precession** sun-synchronous-orbit explainer, and a **Constellation** coverage estimator. **Passes:** a 24-hour **coverage heat map**, the **beta-angle sunlight** threshold, an **Eclipse** lit/shadow timeline, a **Pointing** sky-track, and a **Grid squares** Maidenhead locator tool. **Radio:** an interactive **Transponder** diagram, a two-leg **Doppler** plot, a **link-budget** sandbox, a **full-duplex tuning practice** widget, and an **Antenna** gain pattern. **Reference:** a broad satellite/RF/history reference and a printable four-page **Handouts** classroom set. |
| 30 | **References** | 14 lookup tables: CTCSS tones, Q-codes, CQ and ITU zones, ASCII, ITA2/Baudot, Morse, phonetics, RST, radio maths, the band plan, CubeSatSim C2C, orbit types and amateur-satellite history. |
| 31 | **Exports** | Export the pass schedule to **CSV, Excel, iCal, or JSON** (the iCal events carry a 10-minute reminder alarm); a **multi-satellite comparison** of your favorites; a shareable **per-pass card**; a **Reference orbits** PDF (the first equator crossing of each UTC day for the next 30/60 days — ascending for northern stations, descending for southern — to set up a physical OSCARLOCATOR); and a **Listings** tab with Nova-style tabular ephemerides (one-observer stepped, AOS/LOS quick list, and two-observer stepped), each CSV-exportable. |
| 32 | **Sun / Moon** | Solar and lunar az/el for your site, plus Moon phase and illumination. |
| 33 | **Celestial** | Live az/el of the **Sun, Moon, planets** (Mercury–Saturn) and **cosmic radio sources** (Cassiopeia A, Cygnus A, Crab, Sgr A\*, etc.) plus a **cold-sky** reference and the **selected satellite**, on a sky polar plot and table — for antenna calibration and radio astronomy. A second tab is an **EME (moon-bounce) panel**: Moon az/el and distance, total path loss by band (6 m–3 cm), self-echo Doppler, sky temperature, and **common-Moon-visibility windows** with a second station. CSV-exportable. |
| 34 | **EME** | Moonbounce: path loss, Doppler and common-Moon windows against a DX grid, a **per-band analysis** (Faraday rotation, sky temperature, libration spread, ground gain, Sun separation) and a **90-day planner** by declination and path degradation. |
| 35 | **Space Wx** | Solar 10.7 cm flux, planetary Kp, and A index from NOAA SWPC, with plain-language levels and an operating outlook. Cached for offline viewing. |
| 36 | **MUF / HF Prop** | MINIMUF path MUF from your QTH to 24 world regions, a **shaded world map** of the same model, and **DXCC lookup** by prefix or entity name. Sunspot number is seeded from Space Wx. |
| 37 | **Propagation** | The HF and 6 m operating outlook: headline day/night MUF, per-band open/fair/weak/shut from 80 m to 6 m, geomagnetic state, aurora-VHF likelihood, D-layer absorption, meteor scatter and sporadic-E season. |
| 38 | **Satellites** | The catalog across three tabs: **Catalog** (filter, select, favorite ★, fetch transponders, and **add, edit, or delete manually-entered satellites** by GP mean elements that persist across refreshes); **By type** (the whole catalog grouped by SatNOGS transponder kind — linear / FM / digital / beacon-CW / other — sortable and CSV-exportable); and **What's up now** (a live scan of the whole catalog for satellites above your horizon, sorted by elevation, CSV-exportable). |
| 39 | **New Launches** | **What went up this month that I can actually work?** Crosses the last 30 (or 60) days of the catalog against the SatNOGS transmitter database, setting aside rocket bodies, debris and constellation batches, and offers the intersection as an add-to-favorites list. Anything added keeps updating its elements. |
| 40 | **Sites** | Manage observer locations: nickname the **primary site** (which drives every other screen) and build a table of **secondary sites** (club, portable, friends' QTHs). A **Compare passes** tab shows the selected satellite's upcoming passes across all sites side by side, exportable to CSV or a PDF report. |
| 41 | **Settings** | Set your observer site by lat/lon/altitude or Maidenhead grid, choose the **GP element source** (AMSAT, a CelesTrak category, or a custom OMM-JSON URL), and set the **minimum elevation** used across pass tables and reports. |
...
vector (catalog 88888) to about **one centimeter** at epoch and is accurate for
...
[`sgp4`](https://pypi.org/project/sgp4/) package to well under a kilometer across
a typical pass.
...
It now covers **37 screens** — near parity with the desktop app — including live
tracking, a scrollable pass table, sky radar and world ground-track map, paged
orbital analysis, transponder Doppler dials with a live shift curve, EME,
propagation, orbital history with its own Space-Track fetch, activations, and
the OSCARLOCATOR simulator. Graphics use the terminal's own glyphs: braille
...
See [orbitterm/README.md](orbitterm/README.md) for the screen and key
reference.
...
<p align="center">
  ...
  <img src="docs/img/orbitterm_passes.png" width="48%" alt="OrbitTerm Next Passes: scrollable pass table with quality-colored max elevation">
</p>
<p align="center">
...
The 130+ tests are organized by area under `tests/` (`test_propagation.py`,
...
footprint grazing a border may briefly list a neighbor, which is correct at
